...
class TakeExamView(LoginRequiredMixin, View):
    ...
    def get_exam(self):
        return get_object_or_404(
            Exam.objects.prefetch_related("questions__choice_set"),
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 🔹 Shuffle the questions for each student's attempt
        questions = list(exam.questions.all())  # Convert queryset to a list
        random.shuffle(questions)  # Shuffle in-place

        context = {
            "exam": exam,
            "score": score,
            "expiry_time": score.time_started + timezone.timedelta(minutes=exam.duration),
            "questions": questions,  # Pass shuffled questions to template
        }
        return render(request, self.template_name, context)
    ...
    def post(self, request, *args, **kwargs):
        ...
        for question in questions:
            choice_id = data.get(str(question.id), "")
            if choice_id:
                is_correct = Choice.objects.get(pk=int(choice_id)).is_correct
                choices[question.id] = [choice_id, is_correct]
...
class ExamScoreView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Order by username
        scores = Answer.objects.filter(exam=exam).order_by("user__username")
        ...
        context = {"exam": exam, "scores": scores, "query": query}
        return render(request, self.template_name, context)
